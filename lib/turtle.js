...
class Turtle {
   ...
   * @param  {string} status.id The id of the turtle sending the status update.
  ...
  async updateStatus (status) {
    ...
    if (!this.id === status.id) {
      log.warn('Received status update for a different turtle', { status, turtleId: this.id })
      throw new Error('Received status update for a different turtle')
    }
